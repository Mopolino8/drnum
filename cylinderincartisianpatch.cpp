...
void CylinderInCartisianPatch::operator ()()
{
  ...
  for (size_t layer = 0; layer < 1; ++layer) {
      ...
      for (size_t i = 1; i < m_Patch->sizeI(); ++i) {
        for (size_t j = 0; j < m_Patch->sizeJ(); ++j) {
          ...
          if ((var1[0] > (0.5 + layer) && var2[0] < (0.5 + layer)) ||
              (var1[0] < (0.5 + layer) && var2[0] > (0.5 + layer)))
          {
        ...
        for (size_t j = 1; j < m_Patch->sizeJ(); ++j) {
              ...
              (var1[0] < (0.5 + layer) && var2[0] > (0.5 + layer)))
          {
            ...
            if (var1[0] > (0.5 + layer) && var2[0] < (0.5 + layer)) {
              ...
              real u_layer =  m_Omega*r_2[1];         // ==  m_Omega*norm(r_2)*r_2[1]/norm(r_2);
              real v_layer = -m_Omega*r_2[0];         // == -m_Omega*norm(r_2)*r_2[0]/norm(r_2);
              real weight  = scalarProduct(normalize(r_1), normalize(r_2));
  ...
  for (size_t i = 0; i < m_Patch->sizeI(); ++i) {
    for (size_t j = 1; j < m_Patch->sizeJ(); ++j) {
      for (size_t k = 0; k < m_Patch->sizeK(); ++k) {
        ...
        // To ensure the correct layer values are taken, we choose 1.5, knowing that the cylinder has type value 1.
        if (var1[0] > 1.5) {
          //cout << " in layer x->" << x_p[0] << " y->" << x_p[1] << " z->" << x_p[2] << endl;
          if (var1[0] > 2.5) {
            cout << "data2  " << x_p[0] << "    " << x_p[1] << "    " << x_p[2] << endl;
          } else {
            cout << "data1  " << x_p[0] << "    " << x_p[1] << "    " << x_p[2] << endl;
          }
        ...
        if (var1[0] < 0.5) {
          cout << "data3  " << x_p[0] << "    " << x_p[1] << "    " << x_p[2] << endl;
