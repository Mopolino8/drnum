#ifndef TWO_PATCHES_3_H
#define TWO_PATCHES_3_H


/// @todo test only for debugging interpolates
...
void run()
{
  ...
  //  size_t NJ = 1;
  //  size_t NK = 1*N;
  size_t N = 4;  // needed for time step
  size_t NI = 4;
  size_t NJ = 1;
  size_t NK = 1;
  //.. patch 0
  ...
  x_min = 0.;
  x_max = 3.;  // check this: it should make one exact overlap layers
  size_t NI_0 = NI/2 + 1;
  ...
  // Do not compute, but interpolate only
  real init_var[5];  // init nonsense
  init_var[0] = 0.;
  init_var[1] = 1.;
  init_var[2] = 2.;
  init_var[3] = 3.;
