...
void CartesianRaster::computeDeltas()
{
...
void CartesianRaster::setupAligned(real xo1, real yo1, real zo1, real xo2, real yo2, real zo2)
{
...
void CartesianRaster::buildBoundingBox()
{
...
void CartesianRaster::setupInterpolators()
{
                                                  ...
                                                  WeightedSet<real>& w_set)
{
  ...
  if(m_NumProtectLayers < 1) {
    cout << "CartesianRaster::computeCCDataInterpolCoeffs, m_NumProtectLayers = " << m_NumProtectLayers << endl;
  }
                                                    ...
                                                    WeightedSet<real>& d_dn)
{
  ...
  if(m_NumProtectLayers<2) {
    cout << "CartesianRaster::computeCCGrad1NInterpolCoeffs, m_NumProtectLayers = " << m_NumProtectLayers << endl;
  }
  // Check inside and get reference cell
  inside = xyzToRefInterCell(x, y, z,
			     ic_ref, jc_ref, kc_ref);
...
// void CartesianRaster::computeCCGrad1NInterpolCoeffs(real& x, real& y, real& z,
// 						   size_t& ic_ref, size_t& jc_ref, size_t& kc_ref,
// 						   real nx, real ny, real nz,
// 						   WeightedSet<real>* d_dn)
// {
                                        ...
                                        size_t& ic_ref, size_t& jc_ref, size_t& kc_ref)
{
  ...
  if(inside) {
    ...
    if(ic_ref < m_NumProtectLayers) {
      cout << "CartesianRaster::xyzToRefInterCell, i_min" << endl; error = true;
    }
    if(jc_ref < m_NumProtectLayers) {
      cout << "CartesianRaster::xyzToRefInterCell, j_min" << endl; error = true;
    }
    if(kc_ref < m_NumProtectLayers) {
      cout << "CartesianRaster::xyzToRefInterCell, k_min" << endl; error = true;
    }
    if(ic_ref > m_NumI-2-m_NumProtectLayers) {
      cout << "CartesianRaster::xyzToRefInterCell, i_max" << endl; error = true;
    }
    if(jc_ref > m_NumJ-2-m_NumProtectLayers) {
      cout << "CartesianRaster::xyzToRefInterCell, j_max" << endl; error = true;
    }
    if(kc_ref > m_NumK-2-m_NumProtectLayers) {
      cout << "CartesianRaster::xyzToRefInterCell, k_max" << endl; error = true;
    }
    if(error) {
                                   ...
                                   size_t& ic_ref, size_t& jc_ref, size_t& kc_ref)
{
  ...
  if(inside) {
    ...
    if(ric_ref < 0.) {  // use real ric_ref, due to size_t cycling
      cout << "CartesianRaster::xyzToRefCell, i_min" << endl; error = true;
    }
    if(rjc_ref < 0.) {
      cout << "CartesianRaster::xyzToRefCell, j_min" << endl; error = true;
    }
    if(rkc_ref < 0.) {
      cout << "CartesianRaster::xyzToRefCell, k_min" << endl; error = true;
    }
    if(ic_ref > m_NumI-2) {
      cout << "CartesianRaster::xyzToRefCell, i_max" << endl; error = true;
    }
    if(jc_ref > m_NumJ-2) {
      cout << "CartesianRaster::xyzToRefCell, j_max" << endl; error = true;
    }
    if(kc_ref > m_NumK-2) {
      cout << "CartesianRaster::xyzToRefCell, k_max" << endl; error = true;
    }
    if(error) {
      /// @todo may put this in debug condition
      cout << endl;
...
bool CartesianRaster::xyzToRefNode(real x, real y, real z,
                                   size_t& in_ref, size_t& jn_ref, size_t& kn_ref)
{
  bool inside = true;
  ...
  if(inside) {
    ...
    bool error = false;
    if(rin_ref < 0.) {
      cout << "CartesianRaster::xyzToRefNode, i_min" << endl; error = true;
    }
    if(rjn_ref < 0.) {
      cout << "CartesianRaster::xyzToRefNode, j_min" << endl; error = true;
    }
    if(rkn_ref < 0.) {
      cout << "CartesianRaster::xyzToRefNode, k_min" << endl; error = true;
    }
    if(in_ref > m_NumI-1) {
      cout << "CartesianRaster::xyzToRefNode, i_max" << endl; error = true;
    }
    if(jn_ref > m_NumJ-1) {
      cout << "CartesianRaster::xyzToRefNode, j_max" << endl; error = true;
    }
    if(kn_ref > m_NumK-1) {
      cout << "CartesianRaster::xyzToRefNode, k_max" << endl; error = true;
    }
    if(error) {
      /// @todo may put this in debug condition
      cout << endl;
