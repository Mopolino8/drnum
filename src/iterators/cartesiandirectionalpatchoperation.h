...
public:
  ...
  CartesianDirectionalPatchOperation(CartesianPatch *patch);
...
template <unsigned int DIM, class TFlux>
CartesianDirectionalPatchOperation<DIM, TFlux>::CartesianDirectionalPatchOperation(CartesianPatch *patch) : CartesianPatchOperation(patch)
{
  m_Res = NULL;
  m_ResLength = 0;
}
...
void CartesianDirectionalPatchOperation<DIM, TFlux>::compute(real factor, size_t i1, size_t j1, size_t k1, size_t i2, size_t j2, size_t k2)
{
  ...
  if (i1 == 0) {
    for (size_t j = j1; j < j2; ++j) {
      for (size_t k = k1; k < k2; ++k) {
        fill(flux, 5, 0);
        TFlux::xWallM(patch(), i1, j, k, Ax, flux);
        for (size_t i_var = 0; i_var < DIM; ++i_var) {
  ...
  if (i2 == patch()->sizeI()) {
    for (size_t j = j1; j < j2; ++j) {
      for (size_t k = k1; k < k2; ++k) {
        fill(flux, 5, 0);
        TFlux::xWallP(patch(), i2, j, k, Ax, flux);
        for (size_t i_var = 0; i_var < DIM; ++i_var) {
  ...
  if (j1 == 0) {
    for (size_t i = i1; i < i2; ++i) {
      for (size_t k = k1; k < k2; ++k) {
        fill(flux, 5, 0);
        TFlux::yWallM(patch(), i, j1, k, Ay, flux);
        for (size_t i_var = 0; i_var < DIM; ++i_var) {
  ...
  if (j2 == patch()->sizeJ()) {
    for (size_t i = i1; i < i2; ++i) {
      for (size_t k = k1; k < k2; ++k) {
        fill(flux, 5, 0);
        TFlux::yWallP(patch(), i, j2, k, Ay, flux);
        for (size_t i_var = 0; i_var < DIM; ++i_var) {
  ...
  if (k1 == 0) {
    for (size_t i = i1; i < i2; ++i) {
      for (size_t j = j1; j < j2; ++j) {
        fill(flux, 5, 0);
        TFlux::zWallM(patch(), i, j, k1, Az, flux);
        for (size_t i_var = 0; i_var < DIM; ++i_var) {
  ...
  if (k2 == patch()->sizeK()) {
    for (size_t i = i1; i < i2; ++i) {
      for (size_t j = j1; j < j2; ++j) {
        fill(flux, 5, 0);
        TFlux::zWallP(patch(), i, j, k2, Az, flux);
        for (size_t i_var = 0; i_var < DIM; ++i_var) {
